...
import { getLesson, getNextLesson, LEVEL_COLOR, LEVEL_LABEL } from "@/lib/courses";
...
export default function LearnLessonPage() {
  ...
  const found = getLesson(subjectId, courseId, lessonId);
  const nextLesson = getNextLesson(subjectId, courseId, lessonId);
  ...
  useEffect(() => {
    ...
    fetch("/api/generate-lesson", {
      ...
      body: JSON.stringify({
        ...
        topicName: found.lesson.title,
        topicDescription: found.lesson.description,
      }),
    })
      .then((r) => r.json())
      .then((d) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/learn/progress?subject_id=${subjectId}&course_id=${courseId}`)
      ...
      .catch(() => {});
  }, [isSignedIn, subjectId, courseId, lessonId]);

  async function markComplete() {
    if (!isSignedIn || completed || marking) return;
    setMarking(true);
    try {
      await fetch("/api/learn/progress", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ subject_id: subjectId, course_id: courseId, lesson_id: lessonId }),
      });
      setCompleted(true);
  ...
  const lc = LEVEL_COLOR[found.course.level];
    ...
    <div className="min-h-screen bg-white dark:bg-[#111111]">
      ...
      <div className="mx-auto max-w-2xl px-6 py-12">
        ...
        <div className="mb-8 flex items-center gap-2 text-xs text-[#94A3B8] flex-wrap">
          ...
          <span>/</span>
          <Link href={`/learn/${subjectId}/${courseId}`} className="hover:text-[#64748B] dark:hover:text-white transition-colors">{found.course.title}</Link>
          <span>/</span>
          <span className="text-[#111111] dark:text-white font-medium">{found.lesson.title}</span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          >
            {LEVEL_LABEL[found.course.level]}
          </span>
          <span className="text-[10px] text-[#94A3B8] font-medium uppercase tracking-wider">{found.module.title}</span>
        </div>

        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            <Spinner size="lg" className="text-[#111111] dark:text-white" />
            <p className="text-sm text-[#94A3B8]">Generating lesson on &ldquo;{found.lesson.title}&rdquo;...</p>
          </div>
        ) : error ? (
          <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-8 text-center">
            ...
            <button
              ...
              className="text-sm font-bold text-[#64748B] hover:text-[#111111] dark:hover:text-white transition-colors"
            >
              Try again
            </button>
          </div>
            ...
            <div className="flex flex-col gap-3 pt-2">
              {isSignedIn && (
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full rounded-xl py-4 font-bold text-sm transition-all"
                  style={{
                    background: completed ? "rgba(34,197,94,0.1)" : "#F8FAFC",
                    border: `1px solid ${completed ? "rgba(34,197,94,0.4)" : "#E2E8F0"}`,
                    color: completed ? "#16a34a" : "#64748B",
                  }}
                >
                  <span className="material-symbols-outlined text-lg leading-none">{completed ? "task_alt" : "check_circle"}</span>
                  {completed ? "Lesson Complete!" : marking ? "Saving..." : "Mark as Complete"}
                </button>
              )}

              {nextLesson ? (
                <Link
                  href={`/learn/${subjectId}/${courseId}/${nextLesson.id}`}
                ...
                <Link
                  href={`/learn/${subjectId}/${courseId}`}
                  className="flex items-center justify-center gap-2 w-full rounded-xl bg-[#111111] dark:bg-white text-white dark:text-[#111111] font-bold py-4 hover:opacity-90 transition-all"
                >
                  <span className="material-symbols-outlined leading-none">emoji_events</span>
